...
.readout-inner{font-family:'JBMono';font-weight:500;font-size:20px;letter-spacing:0.14em;line-height:2.3;writing-mode:vertical-rl;color:rgba(0,229,255,0.30);white-space:pre;}
.readout.right .readout-inner{color:rgba(255,47,166,0.30);}
...
.band-content{flex:2.25 1 0;min-height:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:38px;position:relative;}
.bg-icon-huge{position:absolute;top:50%;left:50%;color:rgba(0,229,255,0.05);pointer-events:none;z-index:0;}
.bg-icon-huge svg{width:100%;height:100%;}

/* Typography: Poppins (display) + Inter (body) + JetBrains Mono (readouts,
   tags, eyebrow labels) — off-white with a soft shadow/glow so text holds
   up over the moving scanline/grid texture. */
.headline{font-family:'Poppins',sans-serif;color:#eef8fb;font-size:68px;font-weight:800;text-align:center;line-height:1.14;letter-spacing:-0.03em;width:100%;text-shadow:0 4px 14px rgba(0,0,0,.6), 0 0 26px rgba(0,229,255,0.12);will-change:transform,opacity;z-index:1;}
...
.sub{font-family:'Inter',sans-serif;color:#94b3bd;font-size:37px;font-weight:600;text-align:center;line-height:1.4;width:100%;text-shadow:0 3px 10px rgba(0,0,0,.45);will-change:transform,opacity;z-index:1;}
.mono-label{font-family:'JBMono',monospace;font-weight:700;font-size:27px;letter-spacing:0.14em;color:#5fd8e8;text-transform:uppercase;text-align:center;width:100%;will-change:transform,opacity;z-index:1;}

.tag-row{display:flex;gap:16px;justify-content:center;flex-wrap:nowrap;width:100%;z-index:1;}
.tag-col{display:flex;flex-direction:column;gap:16px;z-index:1;}
.tag{font-family:'JBMono',monospace;display:flex;align-items:center;gap:14px;font-weight:700;font-size:27px;letter-spacing:0.05em;color:#5fd8e8;border:1.5px solid rgba(0,229,255,0.5);padding:20px 26px;background:rgba(0,229,255,0.06);will-change:transform,opacity;}
...
.ring-num{font-family:'Poppins',sans-serif;font-weight:900;font-size:156px;color:#eef8fb;text-shadow:0 0 40px rgba(0,229,255,0.5);}
...
.cta-main{font-family:'Poppins',sans-serif;color:#eef8fb;font-size:56px;font-weight:800;letter-spacing:-0.02em;}
...
<div class="safe" id="safe">
  ...
  <div class="scene" id="sc1">
    <div class="band-content">
      <div style="width:100%;display:flex;align-items:center;justify-content:center;position:relative;gap:40px;">
        <div class="icon-badge" id="s1human" style="width:300px;height:380px;">${humanSvg}</div>
        <div class="panel" id="s1panel" style="padding:24px 28px;display:flex;flex-direction:column;gap:16px;">
          ${tag(I.pencil, 'WRITING', 's1c0')}
          ${tag(I.scissors, 'EDITING', 's1c1', 'magenta')}
          ${tag(I.photo, 'IMAGES', 's1c2')}
        </div>
      </div>
      <div class="mono-label" id="s1status">// STATUS: QUALITY DRIFT DETECTED</div>
      <div class="headline" id="s1head">My ChatGPT workflow works... but the <span class="acc">quality keeps drifting</span>.</div>
    </div>
  </div>
  ...
  <div class="scene" id="sc2">
    <div class="band-content">
      <div class="icon-badge" id="s2visual" style="width:300px;height:368px;color:#eef8fb;">${openai}</div>
  ...
  <div class="scene" id="sc3">
    <div class="band band-tight">
      <div class="headline" id="s3head" style="font-size:60px;">Step 1 — Pick a workflow that needs improvement.</div>
    </div>
    <div class="band-content">
      <div class="bg-icon-huge" id="s3bgicon" style="width:620px;height:620px;margin:-310px 0 0 -310px;">${I.target}</div>
      <div class="tag-row">
  ...
  <div class="scene" id="sc4">
    <div class="band band-tight">
      <div class="headline" id="s4head" style="font-size:60px;">Step 2 — Build a 3-agent review panel.</div>
    </div>
    <div class="band-content">
      <div class="bg-icon-huge" id="s4bgicon" style="width:580px;height:580px;margin:-290px 0 0 -290px;">${I.users}</div>
      <div class="tag-row" style="align-items:stretch;">
        <div class="stack panel" id="s4quality" style="padding:32px 16px;width:236px;">
          <div class="icon-badge" style="width:88px;height:88px;color:#0ff1e0;">${I.target}</div>
          <div class="stack-label">Quality</div>
          <div class="stack-sub">REVIEWER</div>
        </div>
        <div class="stack panel" id="s4prompt" style="padding:32px 16px;width:236px;">
          <div class="icon-badge" style="width:88px;height:88px;color:#5fd8e8;">${I.search}</div>
          <div class="stack-label">Prompt</div>
          <div class="stack-sub">REVIEWER</div>
        </div>
        <div class="stack panel" id="s4expert" style="padding:32px 16px;width:236px;">
          <div class="icon-badge" style="width:88px;height:88px;color:#ff8ed4;">${I.school}</div>
  ...
  <div class="scene" id="sc5">
    <div class="band band-tight">
      <div class="headline" id="s5head" style="font-size:60px;">Step 3 — Define "done" before you start.</div>
    </div>
    <div class="band-content">
      <div class="bg-icon-huge" id="s5bgicon" style="width:600px;height:600px;margin:-300px 0 0 -300px;">${I.circleCheck}</div>
      <div class="panel" id="s5card" style="width:100%;padding:46px 42px;box-sizing:border-box;display:flex;flex-direction:column;gap:26px;">
        <div class="check-row" id="s5c0"><span class="tag-icon">${I.circleCheck}</span><span>Consistent dimensions</span></div>
        <div class="check-row" id="s5c1"><span class="tag-icon">${I.circleCheck}</span><span>Shared visual style</span></div>
        <div class="check-row" id="s5c2"><span class="tag-icon">${I.circleCheck}</span><span>Recognizable details</span></div>
      </div>
  ...
  <div class="scene" id="sc6">
    <div class="band band-tight">
      <div class="headline" id="s6head" style="font-size:60px;">Step 4 — Run exactly <span class="acc">3 loops</span>.</div>
    </div>
    <div class="band-content">
  ...
  <div class="scene" id="sc7">
    <div class="band band-tight">
      <div class="headline" id="s7head" style="font-size:60px;">Step 5 — Test on a brand-new example.</div>
    </div>
    <div class="band-content">
      <div class="bg-icon-huge" id="s7bgicon" style="width:580px;height:580px;margin:-290px 0 0 -290px;">${I.rocket}</div>
      <div class="tag-row" style="align-items:center;">
        <div class="icon-badge" id="s7new" style="width:130px;height:130px;color:#5fd8e8;">${I.sparkles}</div>
        <div class="icon-badge" id="s7arrow" style="width:70px;height:70px;color:#5fd8e8;">${I.arrowRight}</div>
        <div class="icon-badge" id="s7check1" style="width:110px;height:110px;color:#0ff1e0;">${I.circleCheck}</div>
        <div class="icon-badge" id="s7check2" style="width:110px;height:110px;color:#0ff1e0;">${I.circleCheck}</div>
        <div class="icon-badge" id="s7check3" style="width:110px;height:110px;color:#0ff1e0;">${I.circleCheck}</div>
      </div>
  ...
  <div class="scene" id="sc8">
    ...
    <div class="band-content">
      <div class="bg-icon-huge" id="s8bgicon" style="width:660px;height:660px;margin:-330px 0 0 -330px;">${I.bulb}</div>
      <div class="panel tip-panel" id="s8card">
        <span class="tag-icon">${I.bulb}</span>
        <span>Fix the workflow, not the output — or you're patching forever.</span>
      </div>
    </div>
  </div>
  ...
  <div class="scene" id="sc9">
    ...
    <div class="band-content">
      <div class="bg-icon-huge" id="s9bgicon" style="width:660px;height:660px;margin:-330px 0 0 -330px;">${I.bookmark}</div>
      <div class="icon-badge" id="s9bookmark" style="width:250px;height:250px;color:#0ff1e0;">${I.bookmark}</div>
  ...
  <div class="scene" id="sc10">
    <div class="band band-tight">
      <div class="icon-badge" id="s10human" style="width:280px;height:356px;margin:0 auto;">${humanSvg}</div>
    </div>
    <div class="band-content">
      <div class="panel cta-panel" id="s10card">
