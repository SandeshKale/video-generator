...
// Built on GSAP (assets/animations/gsap/gsap.min.js): each helper builds a
// single paused gsap.timeline() per element (memoized on the element, since
// every call site always passes the same params for a given element) and
// scrubs it with tl.progress(e) — never tl.play(). That keeps everything a
// pure function of the e/t passed in from window.__seek, same contract as
// the hand-rolled easing math it replaces, but with GSAP's battle-tested
// bounce/back/power curves instead of copy-pasted formulas.
